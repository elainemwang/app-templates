...
model never sees them when memory is unconfigured. ``create_agent`` composes them into its tool
list. This is a stand-in for a future ``databricks-openai`` memory helper — when the SDK provides
one, swap the import in ``agent.py`` and delete this file.
